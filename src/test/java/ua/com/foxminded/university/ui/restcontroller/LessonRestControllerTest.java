...
class LessonRestControllerTest {
    ...
    class UpdateLessonTest {
        ...
        void putRequestWithAllParametersShouldCallLessonServiceAndRedirect() throws Exception {
            ...
            Lesson lessonAfterUpdating = createTestLesson(ID1);

            when(lessonServiceMock.update(anyInt(), any(Lesson.class)))
                .thenReturn(lessonAfterUpdating);

            mockMvc.perform(put(API_LESSONS_ID, lessonId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(bodyRequest))
                .andDo(print())
            ...
            verify(lessonServiceMock, times(1))
                .update(eq(lessonId), lessonCapture.capture());
